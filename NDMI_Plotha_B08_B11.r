library (terra)
setwd ("/Users/sebastianschmidt/Documents/Uni/Halle/2. Sem/LSS 3 - Plotha/LSS3 Gruppenarbeit/Tiff/SentBrowser/B08_11")
...
b08 <- rasterdateien[seq(1,40,2)] ## die 1., 3. 5., .... Datei bestimmen
b11 <- rasterdateien[seq(2,40,2)] ## die 2., 4. 6., ... Datei bestimmen

band08 <- rast (b08[1]) ## lies die erste Band 08 Datei ein
band11 <- rast (b11[1]) ## lies die erste Band 11 Datei ein
ndmi <- (band08 - band11)/(band08 + band11) ## berechne daraus den ndmi

for (i in 2:length(b08)){ ## Setze der Reihe nach 2,3,4,5,... fuer i ein
  band08 <- rast (b08[i]) ## lies das i-te Band 4 ein
  band11 <- rast (b11[i]) ## lies das i-te Band 8 ein
  ndmi.neu <- (band08 - band11)/(band08 + band11) ## berechne den ndmi fuer diese beiden Baender
...
write.table(plothandmi,"ndmi_plotha_b08_11.csv") ## Schreib die Tabelle auf die Festplatte